...
function normClass(s){ return String(s||'').trim().toLowerCase().replace(/\s+/g,''); }
function teacherSubjectsInClass(userId, cls){
  const out=new Set();
  const nCls=normClass(cls);
  myAssignments(userId).forEach(a=>{ if(String(a.subject||'').trim().toUpperCase()!=='ALL' && normClass(a.class)===nCls) out.add(String(a.subject||'').trim()); });
  return [...out];
}
...
function roleExamAccess(user, exam){
  ...
  if(role==='class_teacher'){
    ...
    const normSet=new Set(mySet.map(normClass));
    const inScope=examClasses.filter(c=> normSet.has(normClass(c)));
...
app.post('/api/exams', verify, authorize('exams'), (req,res)=>{
  ...
  // Accept BOTH numeric class ids AND class names ("P7") — the UI exam modal sends class id
  const allClasses=db.prepare('SELECT id,name FROM classes').all();
  const classByIdMap=new Map(allClasses.map(c=>[c.id,c]));
  const classList=rawClasses.filter(Boolean).map(v=>{
    const n=Number(v);
    if(Number.isInteger(n)&&n>0){
      const r=classByIdMap.get(n);
      return r?r.id:null;
    }
    const r=allClasses.find(c=>String(c.name||'').trim().toLowerCase()===String(v||'').trim().toLowerCase());
    return r?r.id:null;
...
app.get('/api/exams/:id', verify, authorize('exams:read','exams'), (req,res)=>{
  ...
  const accD2=roleExamAccess(req.user,exam); exam.my_access=accD2;
  res.json(exam);
});
